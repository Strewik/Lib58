...
function IssueBookForm({ book, onClose, onSave }) {
  ...
  // Automatically set expected return date as 7 days from issue date
  useEffect(() => {
    if (issueDate) {
  ...
  // Handle form submission for issuing the book
  const handleSubmit = async (e) => {
    ...
    try {
      await api.post("/api/issue/", {
        ...
        status, // Sending status as 'issued'
      });
      onSave(); // Re-fetch book list
      onClose(); // Close modal
